      ...
      >
        <div
          className={`flex items-center rounded-lg px-1.5 py-1.5 w-[95%] transition-colors duration-200 hover:bg-gray-200 rounded-lg ${
            isActive || hoveredIndex === index || activeDropdownIndex == index
              ? 'bg-gray-200'
              : ''
          }`}
        >
...
export default function MyRecent({
  ...
  const getCategorizedChats = useCallback((sessions: RenameSession[]) => {
    ...
    return sessions.reduce(
      (acc: unknown, chat: RenameSession, index: number) => {
  ...
  const renderChatList = useCallback(
    (
      chats: { chat: RenameSession; index: number }[],
      categoryTitle: string,
      renamingChatId: string,
      setRenamingChatId: (name: string) => void,
      handleRename: (sessionId: string, newName: string) => void
    ) => {
